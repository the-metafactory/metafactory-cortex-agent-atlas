...
**Admission.** Atlas admits a task whose `source.channel` is the bound ledger channel **or a thread Atlas itself opened and durably recorded opening** (`owned_threads` in `brain/state.ts`). Before this, a reply typed in a thread was refused *in silence* — including a `RATIFY` Atlas had itself invited — because the shipped Discord adapter puts the **thread's** snowflake in `source.channel` and sends no parent-channel signal at all. "A thread under my channel" is therefore not a question this protocol can answer; "a thread I opened" is the only one it can, which is why the registry exists.

The owned-thread half of admission is gated by the **same** flag that opens threads, so the switch is a real kill switch: turn `ATLAS_THREAD_CONVERSATION` off and, on the next boot, Atlas admits the bound channel and nothing else — whatever the registry already holds. Rows are kept rather than deleted, so turning it back on restores the same threads. The one honest cost of switching it off: a reply typed in a thread Atlas opened earlier is refused in silence, and the conversation has to return to the channel. (Rows never expire on their own and there is no per-thread revoke verb; the flag is the operator control.)
